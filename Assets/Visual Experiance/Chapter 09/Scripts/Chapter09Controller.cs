...
public class Chapter09Controller : MonoBehaviour
{
    ...
    public enum Ch09Status { None, FlyBy, Player1, Player2 };
    public Ch09Status status = Ch09Status.None;
    Ch09Status lastTurn = Ch09Status.Player1;
	...
	void Update ()
    {
		if (!bGameOver)
        {
            ...
            switch(status)
            {
                case Ch09Status.None:
                    break;

                case Ch09Status.Player1:
                ...
                case Ch09Status.Player2:
                    ...
                    else if (!bDoneSetPower)
                    {
                        ...
                        if (Input.GetKeyDown(KeyCode.Space))
                        {
                            ...
                            obj.GetComponent<Chapter09BulletBehaviour>().strStartName = trsPlayer2.tag;
                            obj.GetComponent<Rigidbody>().velocity = obj.transform.up * fStartSpeed;
                        }
                    }
                    if (lastTurn != Ch09Status.Player2)
                        lastTurn = Ch09Status.Player2;
                    break;

                case Ch09Status.FlyBy:
                    break;
            }
    ...
    public void BulletHit()
    {
        if (lastTurn == Ch09Status.Player1)
        {
            status = Ch09Status.Player2;
        }
        else if (lastTurn == Ch09Status.Player2)
        {
            status = Ch09Status.Player1;
        }
    }
}
